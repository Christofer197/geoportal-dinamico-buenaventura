...
async function cargarBarriosParaBusqueda() {
  console.log('🔄 Cargando barrios desde Supabase...');
  try {
    ...
    if (res.ok) {
      ...
      setStatus(`✅ ${barrios.length} barrios cargados - Use el buscador`, '#27ae60');
    } else {
      console.error('❌ Error HTTP al cargar barrios:', res.status, res.statusText);
      setStatus('⚠️ Error cargando barrios desde base de datos', '#e74c3c');
    }
  } catch (err) {
    console.error('❌ Error cargando barrios:', err);
    setStatus('⚠️ Error de conexión con base de datos', '#e74c3c');
  }
}
...
function configurarBusqueda() {
  ...
  console.log('✅ Búsqueda de barrios configurada');
  ...
  input.addEventListener('input', (e) => {
    ...
    if (barrios.length === 0) {
      results.innerHTML = '<div class="search-result-item">⏳ Cargando barrios...</div>';
